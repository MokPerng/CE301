...
function BattleScene() {
  ...
  const minLevel = 5;
  const maxLevel = 10; // can set max level
  ...
  const levelDiff = randomLevel - minLevel;
    ...
    if (levelDiff > 0) {
      ...
      randomMonster.stats = randomMonster.stats.map((stat, index) => stat + randomMonster.level_up[index] * levelDiff),
  ...
  document.querySelectorAll('button').forEach((button) => {
    button.addEventListener('click', (e) => {
      ...
      if (enemy.health <= 0) {      //enemy faint check
        ...
        queue.push(() => {
          Exp_get = enemy.level * 20     // exp system can be change
